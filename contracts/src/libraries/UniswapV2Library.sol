...
library UniswapV2Library {
    // returns sorted token addresses, used to handle return values from pairs sorted by token address
    ...
    // fetches the current cumulative prices from the pair
    function currentCumulativePrices(address pair) internal view returns (uint price0Cumulative, uint price1Cumulative, uint32 blockTimestamp) {
        ...
        (, , blockTimestamp) = _pair.getReserves();
    }
}
