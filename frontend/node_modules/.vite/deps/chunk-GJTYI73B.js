...
import {
  base64ToString,
  base64UrlToBase64
} from "./chunk-43F4DZXD.js";

// node_modules/@passwordless-id/webauthn/dist/browser/webauthn.min.js
var x = Object.defineProperty;
var y = (e, a) => {
  for (var t in a) x(e, t, { get: a[t], enumerable: true });
};
var C = {};
y(C, { authenticate: () => H, isAutocompleteAvailable: () => E, isAvailable: () => V, isLocalAuthenticator: () => L, register: () => k });
var s = {};
y(s, { bufferToHex: () => P, concatenateBuffers: () => O, isBase64url: () => S, parseBase64url: () => o, parseBuffer: () => K, sha256: () => A, toBase64url: () => c, toBuffer: () => b });
function b(e) {
...
async function k(e) {
  ...
  if (!S(e.challenge)) throw new Error("Provided challenge is not properly encoded in Base64url");
  let a = typeof e.user == "string" ? { name: e.user } : e.user;
  a.id || (a.id = crypto.randomUUID());
  let t = { challenge: o(e.challenge), rp: { id: e.domain ?? window.location.hostname, name: e.domain ?? window.location.hostname }, user: { id: b(a.id), name: a.name, displayName: a.displayName ?? a.name }, hints: e.hints, pubKeyCredParams: [{ alg: -7, type: "public-key" }, { alg: -257, type: "public-key" }], timeout: e.timeout, authenticatorSelection: { userVerification: e.userVerification, authenticatorAttachment: v(e.hints), residentKey: e.discoverable ?? "preferred", requireResidentKey: e.discoverable === "required" }, attestation: "direct", ...e.customProperties };
  console.debug(t), d == null ? void 0 : d.abort("Cancel ongoing authentication"), d = new AbortController();
  let i = await navigator.credentials.create({ publicKey: t, signal: d == null ? void 0 : d.signal }), r = i.response;
  if (d = null, console.debug(i), i.type != "public-key") throw "Unexpected credential type!";
  let n = r.getPublicKey();
  if (!n) throw "Non-compliant browser or authenticator!";
  return { type: i.type, id: i.id, rawId: c(i.rawId), authenticatorAttachment: i.authenticatorAttachment, clientExtensionResults: i.getClientExtensionResults(), response: { attestationObject: c(r.attestationObject), authenticatorData: c(r.getAuthenticatorData()), clientDataJSON: c(r.clientDataJSON), publicKey: c(n), publicKeyAlgorithm: r.getPublicKeyAlgorithm(), transports: r.getTransports() }, user: a };
}
async function E() {
...
function u(e) {
  ...
  return { rpIdHash: $(e), flags: { userPresent: !!(a & 1), userVerified: !!(a & 4), backupEligibility: !!(a & 8), backupState: !!(a & 16), attestedData: !!(a & 64), extensionsIncluded: !!(a & 128) }, signCount: new DataView(e.slice(33, 37)).getUint32(0, false), aaguid: G(e) };
}
function $(e) {
  return c(e.slice(0, 32));
}
function G(e) {
  if (e.byteLength < 53) return "00000000-0000-0000-0000-000000000000";
  let a = e.slice(37, 53), t = P(a);
...
function X(e) {
  let a = u(e.response.authenticatorData);
  return N(e, a);
}
function N(e, a) {
  let t = a.aaguid;
  return { authenticator: { aaguid: t, counter: a.signCount, icon_light: "https://webauthn.passwordless.id/authenticators/" + t + "-light.png", icon_dark: "https://webauthn.passwordless.id/authenticators/" + t + "-dark.png", name: p[t] ?? "Unknown" }, credential: { id: e.id, publicKey: e.response.publicKey, algorithm: B(e.response.publicKeyAlgorithm), transports: e.response.transports }, synced: a.flags.backupEligibility, user: e.user, userVerified: a.flags.userVerified };
}
function I(e, a) {
  return { credentialId: e.id, userId: e.response.userHandle, counter: a.signCount, userVerified: a.flags.userVerified, authenticatorAttachment: e.authenticatorAttachment };
}
function z(e) {
  let a = u(e.response.authenticatorData);
  return I(e, a);
...
async function Z(e, a) {
  let t = g(e.response.clientDataJSON), i = u(e.response.authenticatorData);
  if (!i.aaguid) throw new Error("Unexpected error, no AAGUID.");
  if (t.type !== "webauthn.create") throw new Error(`Unexpected ClientData type: ${t.type}`);
  if (await F(a.origin, t.origin)) throw new Error(`Unexpected ClientData origin: ${t.origin}`);
  if (await F(a.challenge, t.challenge)) throw new Error(`Unexpected ClientData challenge: ${t.challenge}`);
  return l.toRegistrationInfo(e, i);
}
async function _(e, a, t) {
  if (e.id !== a.id) throw new Error(`Credential ID mismatch: ${e.id} vs ${a.id}`);
  if (!await U({ algorithm: a.algorithm, publicKey: a.publicKey, authenticatorData: e.response.authenticatorData, clientData: e.response.clientDataJSON, signature: e.response.signature, verbose: t.verbose })) throw new Error(`Invalid signature: ${e.response.signature}`);
  let r = g(e.response.clientDataJSON), n = u(e.response.authenticatorData);
  if (t.verbose && (console.debug(r), console.debug(n)), r.type !== "webauthn.get") throw new Error(`Unexpected clientData type: ${r.type}`);
  if (await F(t.origin, r.origin)) throw new Error(`Unexpected ClientData origin: ${r.origin}`);
  if (await F(t.challenge, r.challenge)) throw new Error(`Unexpected ClientData challenge: ${r.challenge}`);
  let f = t.domain ?? new URL(r.origin).hostname, h = c(await A(b(f)));
  if (n.rpIdHash !== h) throw new Error(`Unexpected RpIdHash: ${n.rpIdHash} vs ${h}`);
  if (!n.flags.userPresent) throw new Error("Unexpected authenticator flags: missing userPresent");
  if (!n.flags.userVerified && t.userVerified) throw new Error("Unexpected authenticator flags: missing userVerified");
  if (t.counter && n.signCount <= t.counter) throw new Error(`Unexpected authenticator counter: ${n.signCount} (should be > ${t.counter})`);
  return I(e, n);
}
function T(e) {
  switch (e) {
...
async function R(e, a) {
  let t = T(e), i = o(a);
  return crypto.subtle.importKey("spki", i, t, false, ["verify"]);
}
async function U({ algorithm: e, publicKey: a, authenticatorData: t, clientData: i, signature: r, verbose: n }) {
  let f = await R(e, a);
  n && console.debug(f);
  let h = await A(o(i)), w = O(o(t), h);
  n && (console.debug("Algorithm: " + e), console.debug("Public key: " + a), console.debug("Data: " + c(w)), console.debug("Signature: " + r));
  let D = o(r);
  e == "ES256" && (D = q(D));
  let Y = T(e);
  return await crypto.subtle.verify(Y, f, D, w);
}
function q(e) {
  let a = new Uint8Array(e), t = a[4] === 0 ? 5 : 4, i = t + 32, r = a[i + 2] === 0 ? i + 3 : i + 2, n = a.slice(t, i), f = a.slice(r);
  return new Uint8Array([...n, ...f]);
}
...
var PasskeyWebClient = class {
  ...
  async register(args) {
    ...
    const registration = await C.register({
      attestation: true,
      challenge,
      domain: rp.id,
      user: name,
    ...
    return {
      ...
      credential: {
        algorithm: l.getAlgoName(registration.response.publicKeyAlgorithm),
...
//# sourceMappingURL=chunk-GJTYI73B.js.map
